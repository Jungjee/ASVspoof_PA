...
class CenterLoss(nn.Module):
	...
	"""
	def __init__(self, num_classes = None, feat_dim = None, use_gpu = True):
...
if __name__ == '__main__':
	...
	#get 4 utt_lists
	l_gen_trn, l_spo_trn= get_utt_list(parser['DB'] + 'spec_trn_torch/')
	l_gen_dev, l_spo_dev= get_utt_list(parser['DB'] + 'spec_dev_torch/')
	...
	devset = Dataset_ASVspoof2019_PA(list_IDs = l_dev_utt,
		...
		base_dir = parser['DB'] + 'spec_dev_torch/')
	...
	if parser['optimizer'].lower() == 'sgd':
		params = list(model.pararmeters()) + list(c_obj_fn.parameters())
	...
	for epoch in tqdm(range(parser['epoch'])):
		...
		trnset = Dataset_ASVspoof2019_PA(list_IDs = trn_list_cur,
			labels = d_label_trn,
			nb_time = parser['nb_time'],
			base_dir = parser['DB'] + 'spec_trn_torch/')
